...
export function buildConfig(pluginConfig = {}) {
  ...
  const configuredUserId = cfg.userId ?? loadEnvVar("MEMOS_USER_ID");
  const hasConfiguredUserId =
    configuredUserId !== undefined && configuredUserId !== null && String(configuredUserId).trim() !== "";
  const userId = hasConfiguredUserId ? String(configuredUserId).trim() : "openclaw-user";
  ...
  const conversationSuffixMode = normalizeEnum(
    cfg.conversationSuffixMode ?? loadEnvVar("MEMOS_CONVERSATION_SUFFIX_MODE"),
    ["none", "counter"],
    "none",
  );
  ...
  const relativity = clampNumber(
    cfg.relativity ?? loadEnvVar("MEMOS_RELATIVITY") ?? 0.45,
    0,
    1,
    0.45,
  );
  const memoryEnabled = parseBool(
    cfg.memoryEnabled ?? loadEnvVar("MEMORY_ENABLED"),
    true,
  );
  const memoryTopK = clampInt(
    cfg.memoryTopK ?? loadEnvVar("MEMORY_TOP_K") ?? cfg.memoryLimitNumber ?? 5,
    1,
    50,
    5,
  );
  const memoryBudgetTokens = clampInt(
    cfg.memoryBudgetTokens ?? loadEnvVar("MEMORY_BUDGET_TOKENS") ?? 800,
    100,
    8000,
    800,
  );
  const memorySearchTimeoutMs = clampInt(
    cfg.memorySearchTimeoutMs ?? loadEnvVar("MEMORY_SEARCH_TIMEOUT_MS") ?? cfg.timeoutMs ?? 1000,
    100,
    30000,
    1000,
  );
  const memoryWriteAsync = parseBool(
    cfg.memoryWriteAsync ?? loadEnvVar("MEMORY_WRITE_ASYNC") ?? cfg.asyncMode,
    true,
  );
  const memoryWriteRetry = clampInt(
    cfg.memoryWriteRetry ?? loadEnvVar("MEMORY_WRITE_RETRY") ?? cfg.retries ?? 2,
    0,
    5,
    2,
  );
  const memoryCacheTtlSec = clampInt(
    cfg.memoryCacheTtlSec ?? loadEnvVar("MEMORY_CACHE_TTL_SEC") ?? 60,
    0,
    3600,
    60,
  );
  const memoryScopeMode = normalizeEnum(
    cfg.memoryScopeMode ?? loadEnvVar("MEMORY_SCOPE_MODE"),
    ["user", "chat", "hybrid"],
    "hybrid",
  );
  const memoryPiiFilterEnabled = parseBool(
    cfg.memoryPiiFilterEnabled ?? loadEnvVar("MEMORY_PII_FILTER_ENABLED"),
    true,
  );
  const memoryDegradeOnError = parseBool(
    cfg.memoryDegradeOnError ?? loadEnvVar("MEMORY_DEGRADE_ON_ERROR"),
    true,
  );
  const memoryGrayPercent = clampInt(
    cfg.memoryGrayPercent ?? loadEnvVar("MEMORY_GRAY_PERCENT") ?? 100,
    0,
    100,
    100,
  );
  const tenantId = cfg.tenantId ?? loadEnvVar("MEMORY_TENANT_ID") ?? "default";
  const channel = cfg.channel ?? loadEnvVar("MEMORY_CHANNEL") ?? "openclaw";
